...
echo set /p input=What file extention would you like to add to the New menu: %%^=%%>>temp.bat
echo REG ADD HKEY_CLASSES_ROOT\%%input%%\ShellNew>>temp.bat
echo REG ADD HKEY_CLASSES_ROOT\%%input%%\ShellNew /v NullFile>>temp.bat
echo echo Added %%input%% to the New list!>>temp.bat
echo pause>>temp.bat
